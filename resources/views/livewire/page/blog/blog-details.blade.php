    ...
    <section id="blog" class="p-5" >
        <div class="container  text-center p-sm-1">
            <div class="align-items-center">
                <h3 class="h1 pb-5 text-uppercase">BLOG DETAILS</h3>
            </div>
            <div class="row">
                    ...
                    @if($blog_data)
                        @foreach($blog_data as $key => $value)
                            <div class="col-12 p-0 m-0 shadow">
                                <div class="row">
                                    <div class="col">
                                        <img class=""  alt="Hanrickson E. Dumapit Blogs {{$value->title}}" src="{{asset('storage/content/blogs/'.$value->image)}}" alt="" style="object-fit:cover;width:inherit" >
                                    </div>
                                </div>
                                <div class="row">
                                    ...
                                    <div class="col-12">
                                        <div style="width:inherit;overflow: hidden;text-overflow: ellipsis;white-space: nowrap; " >
                                            @foreach($this->tag_data as $tag_key => $tag_value)
                                                <a wire:navigate href="/blog/@if(isset($this->user_id) && $this->user_id !=1 ){{$this->user_id.'/'}}@endif{{'tag/'.$tag_value->tag_id}}">
                                ...
                                <div class="row ">
                                    <div class="col-12 text-start px-5 py-4 d-flex justify-content-end">
                                        <a class="btn @if($mode == 1) btn-primary @else btn-outline-primary @endif " wire:navigate href="blog/@if(isset($user_id)){{$user_id.'/'}}@endif{{$value->id}}">{{$value->button}}</a>
                                    </div>
                                </div>
                                @endif
                            </div>
                    ...
                    <div class="col-lg-3 col-md-12 col-sm-12 ">
                        <div class="col-12 px-4 shadow">
                            <div class="row">
                                ...
                                <a  wire:navigate href="/blog/@if(isset($user_id)){{$user_id.'/'}}@endif{{'tag/'.$value->tag_id}}" class="link-underline link-underline-opacity-0 @if($mode == 0) text-white @else text-dark @endif">
                                    <div class="row">
